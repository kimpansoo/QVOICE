        ...
        with sr.Microphone() as raw_voice:
            ...
            try:
                ...
                if (voice_data == backup_open_list_port[0][0]):label2.config(foreground="red")
                    ...
                    ser.write('G'.encode())
                    label5.config(foreground="gray15")
...
def data_del_port(): #삭제하기
    ...
    if MsgBox == 'yes':
        speech_1_on_port = []
        backup_list_port = []
        backup_open_list_port = [["켜기 음성 입력하세요", "끄기 음성 입력하세요"],["켜기 음성 입력하세요", "끄기 음성 입력하세요"],["켜기 음성 입력하세요", "끄기 음성 입력하세요"]]
        con1_on.delete(0, END)
        con2_on.delete(0, END)
        con3_on.delete(0, END)
        con1_off.delete(0, END)
...
root = Tk()
root.title("음성 제어기 QVOICE_Ver3.0")
root.geometry("910x200+10+10")
...
label5 = Label(root, text="선풍기", font=("고딕", 10))
label5.grid(column=7, row=5 )
lb5 = Label(root, text="(선풍기 켜, 선풍기 꺼, 선풍기 1단, 선풍기 2단,\n선풍기 3단, 선풍기 4단, 선풍기 회전)", font=("고딕", 9))
lb5.grid(column=8, columnspan=2,row=5 )
...
#btn_back = Button(root, text="적용/백업", state=tk.DISABLED, width=8, font=("고딕", 10), bg="pink", command=data_back)
#btn_back.grid(column=1, row=2, padx=5, 인터넷 닫기 인터넷 닫기 pady=1)
...
lb = Listbox(root,width=92, height=8,bd=0 ,highlightthickness=0 ,font=("고딕",9))
